...
then
	...
	then
	    ...
	    mv build/out/qbe-*.tar.gz build/out/src/qbe-*.tar.gz ../qtum-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    mv build/out/qbe-*-win-unsigned.tar.gz inputs/qbe-win-unsigned.tar.gz
	    mv build/out/qbe-*.zip build/out/qbe-*.exe ../qbe-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    mv build/out/qbe-*-osx-unsigned.tar.gz inputs/qbe-osx-unsigned.tar.gz
	    mv build/out/qbe-*.tar.gz build/out/qbe-*.dmg ../qtum-binaries/${VERSION}
	fi
	popd
...
then
	...
	then
	    ...
	    mv build/out/qbe-*win64-setup.exe ../qtum-binaries/${VERSION}
	    mv build/out/qbe-*win32-setup.exe ../qtum-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    mv build/out/qbe-osx-signed.dmg ../qtum-binaries/${VERSION}/qbe-${VERSION}-osx.dmg
	fi
	popd
